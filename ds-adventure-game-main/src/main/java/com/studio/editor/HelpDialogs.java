...
final class HelpDialogs {
    ...
    private static String flowHelp() {
        return """
                ...
                引擎自动信号（不需要声明 signal，直接写槽就行；场景属性窗口「信号 / 槽」页有 🔔 一键按钮）：
                  slot = 场景进入       | …   ← 每次进入本场景、节点建好后执行
                                               参数：scene=当前场景　from=上一个场景名（第一幕为空）
                  slot = 场景离开       | …   ← 即将离开本场景时执行（此时旧场景的节点还在，能改/能存/能停表）
                                               参数：scene=被离开的场景　to=即将进入的场景
                  slot = 上一个场景离开 | …   ← 新场景渲染完成后发到「新的一幕」（紧跟在「场景进入」之后）
                                               参数：scene=被离开的场景　to=当前场景
                                               用途：在新场景里对“刚走的那一幕”做反应
                                               （记录来路、接续上一幕的倒计时、播转场……）
                  别名：进入场景 / 场景开始、离开场景 / 场景结束、上一幕离开 / 上个场景离开
                  顺序：场景离开（旧幕）→ 场景进入（新幕）→ 上一个场景离开（新幕）
                  常见用法：进场景自动存档、自动刷新派生变量、自动播 BGM、起一个定时器。

                槽里“发信号”与“定时器”（不需要写 Java，全用自带插件）：
                  slot = 刷新     | emit |          | 我发出的信号      ← 手动发信号（目标留空=发到本场景）
                  slot = 场景进入 | @plugin(after)  | 3  | 定时到       ← 3 秒后发一次「定时到」
                  slot = 场景进入 | @plugin(every)  | 1  | 每秒         ← 每 1 秒发一次「每秒」
                  slot = 停止     | @plugin(stoptimer) | 每秒          ← 停掉某个定时器（留空=全部）
                  例：进场景 → 起定时器 → 定时器结束发信号 → 槽里收到后 goto 下一幕（示例地图 demo_clock 就是这么做的）
                  注意：定时器跟着插件生命周期；换场景时建议在自己的「场景离开」里把它停掉。
    ...
    private static String shortcutHelp() {
        return """
                ...
                场景 → 🧩 场景属性（属性 / 节点 / 信号槽）…         一个窗口看全场景：属性表、节点列表、场景信号与槽
                   ...
                   · 信号/槽页：一行式文本查看/新增/修改/删除场景级信号与槽，另外还有两排快捷按钮 ——
                     「＋鼠标点击 / ＋按键 F / ＋空格键 / ＋方向键↑↓」一键加常用信号；
                     「🔔 场景进入 / 🔔 场景离开 / 🔔 上一个场景离开」一键订阅引擎自动信号（会先加一条 log 槽，改成自己的逻辑即可）；
                     「＋发信号槽 / ＋定时槽(after) / ＋循环槽(every) / ＋停定时槽 / ＋改属性槽 / ＋逻辑槽 / ＋跳场景槽 / ＋日志槽」
                     点一下就直接多一条槽；插件下拉框（可打字筛选）里选中再「＋ 插入插件槽」把动词换成自己需要的
                文件 → 🎁 生成演示地图 → 倒计时时钟                        示例：场景自动信号 + 槽里定时器发信号 +
                                                                  倒计时结束自动跳下一幕（maps/demo_clock）
