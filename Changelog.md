...
- 剧本：按 **K=6** 写入 **97 条 `@exit`**（11 个文件，纯新增行；`git checkout -- "docs/ds-adventrue/剧本"` 回滚）
- 报告：`docs/ds-adventrue/常驻立绘清理-干跑报告.md`（逐段明细 + K 敏感性 + 按角色统计）

### Verified
- 编译产物实测：全篇平均同台 **5.37 → 2.00**（每章都降；峰值 8 → 4~5；终章 7.0 → 1.0）
- 立绘引用 **4991 → 1825**（−63%）；地图 103844 → 72708 行
- **顺带修好了「立绘放大」**：1~2 人拍（bust/mid 生效）从 **9% → 88%**，crowd 档从 4025 → 36 个节点
  —— 之前立绘被迫缩小，正是因为没戏的角色一直占着舞台
- `mvnw test` **155/155**；A/B 对照：只留段落兜底 = 平均 2.40 / **0 次闪回**；
  含窗口 exit = 2.00 / 51 次「闪回」（该角色本段内还会再开口）→ 按你选的 K=6 采用后者

### Known gaps
- 51 处「闪回」：窗口退场后角色在本段内又开口 → 淡出再淡入；不喜欢就撤掉窗口 exit（改走 2.40/0 闪回）
