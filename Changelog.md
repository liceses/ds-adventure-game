# 更新日志 (Changelog)
## [v1.24] 第6章演出微调：初音未来站 C 位 + 摆位不再把人推出画外

### Fixed
- **「初音未来的站位太靠外了，看不见」**（实机反馈）：第 6 章初音写在 `@enter miku ... left`，
  而 ds 娘整章也在 `left` —— 编译器的「同站位错开」对 `left` 一律**朝画面外**偏移，
  初音被推到**角色中心 = −14px**（整只几乎出画）。
- **同一个 bug 波及全篇**：实测 **135** 个立绘节点中心顶出画面
  （戏官 39 / 初音 38 / 灯官 28 / GLM 娘 18 / 皮卡丘 7 / ds 娘 5）——
  只要两个人占同一站位就必然出画，跟角色是谁无关。
- **司秤吏在序章是一块白色占位块**：序章 L12 他就开口了，而编译器给"自动上场"角色的默认表情是
  `base`，`sprites/sclerk/` 里只有 `silhouette.png` / `panic.png` → 期望路径不存在 → 引擎画占位块
  （21 拍）。第 6 章也踩过一次（写 `@enter sclerk normal` 时）。

### Changed
- `tools/build_story.mjs`：同站位错开改为**朝画面内侧**（left 往右 / right 往左 / center 左右交替），
  并加**安全带夹取** `clampBodyCenter`（角色中心线至少离画面边 1/4 框宽，框越大人越粗）
- `tools/build_story.mjs`：新增**缺图告警** —— 立绘候选全部落空时报出「角色/表情 + 该目录现有素材」，
  不再默默生成一个不存在的路径
- 剧本 `第6章_歌姬的曲库灾难.txt`：初音未来全章 **C 位（center）**；ds 娘固定 `left`；
  报幕虫改为右侧报幕后即退场（不再整章占着舞台）；补 `@enter sclerk silhouette right` 两处
- 剧本 `第6章...`：初音登场用上 **`@debut miku`**（全身单独立绘卡拍、点一下继续）——
  兑现 v1.22 里"角色初次登场来个全身立绘展示"的能力
- 剧本 `序章_404之夜.txt`：司秤吏首次开口前补 `@enter sclerk silhouette left`

### Verified
- 立绘节点 **2135** 个：身体中心出画 **135 → 0**
- 第 6 章 C 位占用：初音 **71 拍**（ds 娘 2 拍）；截图复核 `_shots/ch6/`（before/after 成对）
- 新单测 `StagePlacementTest`（2 条）：全篇立绘中心线在安全带内 + 立绘文件都存在 ——
  故意把初音 `x` 改成 −600 后测试立刻红（证明不是空测）
- `mvnw test` **157/157**（155 + 2）；`node tools/build_story.mjs --check` 零差异

## [v1.23] 常驻立绘清理：段落兜底清场 + 说话人自动上场（平均同台 5.37 → 2.40）

### Fixed
- **立绘只进不出**（实机反馈：「越往后面常驻在画面上的人物立绘越多，她们根本没有剧情」）：
  全篇 `@enter` **242** 次、`@exit` 只有 **16** 次，而编译器 `case "scene"` **从不清 `stage.chars`**
  → 立绘一旦进场就永远在场上：平均同台从第1章 **1.9** 一路涨到第8章 **7.6**，终章恒定 7 人
  （其中 reimu/paimon/creeper/miku/sai 整章零台词却一直站着）

### Added
- `tools/plan_cast_exits.mjs`：常驻立绘诊断 + 退场计划器（段落边界清场 + 段内 **K 拍**窗口 +
  前瞻保护；`--markdown` 出干跑报告、`--apply` 写回剧本、K 敏感性对照）
- **编译器兜底**：进入新 `@label` 时把「本段整段没有台词」的立绘清掉（`@enter ... hold` 豁免），
  防止以后新写的段落再累积；**说话人自动上场**（剧本没 `@enter` 但说了话 → 自动补上场 + 编译期提示）
- 剧本：**不改写**（一度按 K=6 写入 97 条 `@exit`，实机评估后撤掉 —— 见下「策略取舍」）；
  要再积极一点随时可恢复：`node tools/plan_cast_exits.mjs --k=6 --apply`
  （撤销：`git checkout <commit> -- "docs/ds-adventrue/剧本"`）
- 报告：`docs/ds-adventrue/常驻立绘清理-干跑报告.md`（逐段明细 + K 敏感性 + 按角色统计）

### Verified
- 编译产物实测（**最终采用「只做段落级清场」**）：全篇平均同台 **5.37 → 2.40**
  （每章都降；峰值 8 → 4~6；终章 7.0 → 1.0）
- 立绘引用 **4991 → 2212**（−56%）；地图 103844 → 76054 行
- **顺带修好了「立绘放大」**：bust 档节点 **172 → 719**、crowd 档 **4025 → 174**
  —— 之前立绘被迫缩小，正是因为没戏的角色一直占着舞台
- `mvnw test` **155/155**
- 策略取舍（A/B 实测）：① **段落级清场 = 2.40 / 0 闪回**（采用）；② 再加「段内 K=6 窗口退场」
  = 2.00 / **51 处闪回**（角色本段内又开口 → 淡出再淡入）→ 实机评估后放弃窗口退场

### Known gaps
- 段落级清场是「换段换人」：同一段里沉默的配角会站到本段结束（要单独请下台就在剧本里手写 `@exit`）；
  段内沉默者不再被自动请走，所以完全没有「闪回」
- 说话人自动上场用的是「最近一次表情 + 空闲站位」，建议剧情侧补写 `@enter` 指定表情
  （编译期会点名：bugs、creeper、ds、qianwen、sai、sclerk、戏官、灯官、皮卡丘、秤主）
- 段落兜底只按「本段有无台词」判断；要「故意让某人沉默地站着」得写 `@enter ... hold`
