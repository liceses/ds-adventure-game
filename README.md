...
- **启动顺序**（免得你以为在玩最新版）：① 有本机编译产物 `target\ds-adventure.jar` → 那是**当前源码**的构建，优先玩它；
  ② 没有（`mvnw clean` 会清掉它）→ **现在就编译**；③ 只有本机编不出来（没有 JDK / 编译失败）才退回
  `dist\` 里的**打包版 exe**，并且会**打印它的构建时间**提醒你那是更早源码树的快照。
